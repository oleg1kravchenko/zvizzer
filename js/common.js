...
billbords.forEach(billbord => {
  const cards = billbord.querySelectorAll('.billbord__img');
  ...
  billbord.addEventListener('mousemove', (e) => {
    ...
    cards.forEach(card => {
      card.style.transform = `perspective(${clientWidth}px) rotateY(${rotateX}deg) scale3d(1, 1, 1)`;
    });
  });

  billbord.addEventListener('mouseleave', (e) => {
    const { currentTarget } = e;
    const clientWidth = currentTarget.clientWidth;

    cards.forEach(card => {
      card.style.transform = `perspective(${clientWidth}px) rotateX(0deg) rotateY(0deg)`;
    });
  });
});
